  ...
  var _class_ = function _class_(Target) {
    ...
    function (_Component) {
      ...
      function Effect(props) {
        ...
        _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "createActions", function (obj) {
          var actions = {};

          for (var name in obj) {
            if (obj.hasOwnProperty(name) && (0, _utils.isFn)(obj[name])) {
              actions[name] = _this.createAction(name, obj[name]);
            }
          }
          ...
          return names.reduce(function (buf, name) {
            if (typeof name === 'object') {
              var _loop = function _loop(key) {
                if (name.hasOwnProperty(key) && (0, _utils.isFn)(name[key])) {
                  buf[key] = function () {
                    ...
                    if (res !== undefined) {
                      _this.dispatch(key, res);
                    } else {
                      var _this2;

                      (_this2 = _this).dispatch.apply(_this2, [key].concat(args));
                    }
                  };
                }
              };
              ...
              buf[name] = function () {
                ...
                (_this3 = _this).dispatch.apply(_this3, [name].concat(args));
              };
            }
      ...
      _proto.runEffects = function runEffects(props) {
        ...
        if ((0, _utils.isFn)(effects)) {
          effects(function (type, $filter) {
            if (!_this4.subscribes[type]) {
              _this4.subscribes[type] = new _rxjs.Subject();
            }

            if ((0, _utils.isFn)($filter)) {
              return _this4.subscribes[type].pipe((0, _operators.filter)($filter));
            }

            return _this4.subscribes[type];
          });
        }
      };
